...
Most of these dictionaries were not developed and validated for news texts, but researchers still use them in news analysis. This off-the-shelf dictionary-based sentiment analysis has been used quite heavily in political communication literature [e.g. @Balmas2017; @Boukes2019; @Haselmayer2016; @Soroka2015; @Young2012]. New dictionaries such as Lexicoder [@Young2012], VADER [@gilbert2014vader] and crowd-sourcing-based sentiment dictionaries [@Haselmayer2016] were developed for application in communication science.
...
Although our hypothesis is derived from @cohen:2004:ini, the hypothesis of the analysis in our p-hacking experiment is different in three respects. Firstly, Cohen's argument [-@cohen:2004:ini] is based on an analysis of the annually aggregated news tone of presidential news in the NYT [@ragsdale1997disconnected] and the annually aggregated presidential approval from Gallup Polls. Our hypothesis is not limited to presidential news and instead extends across all news. Secondly, there are differences in operationalization of news tone. The aggregated news tone used by @cohen:2004:ini was hand-coded, but not computationally extracted as in the current study. Finally, we used a Granger causality test (the standard method to study agenda setting), instead of simple linear regression as in @cohen:2004:ini.

We would like to emphasize that the purpose of this study is **not** to replicate or extend Cohen's argument. Instead, we use our hypothesis as a case study to demonstrate the properties of sentiment scores based on off-the-shelf sentiment dictionaries and the risks of using them in domain applications without first establishing their validity for addressing the study's research questions [as proposed in @atteveldt:2018:wcm]. Thus, we have no 'ground truth' and do not present a theoretical expectation on how the two variables (news sentiment and presidential approval) should behave; thus, we do not consider which p-value from our p-hacking experiment is 'wrong'. Instead, we aim to demonstrate that a large variety of conclusions can be derived using these dictionaries (which could be cherry picked) and the possible explanations behind this high variety of conclusions.
...
The NYT data for this study was collected from ProQuest Historical Newspapers. We selected the NYT instead of another newspaper because it is an American 'newspaper of record'. We used the date of publication, content length (number of words) and sentiment scores extracted from the NYT corpus. The articles represented the entire publication output of the NYT from June 1, 1980 to January 31, 2006. All articles were converted to lowercase and tokenized. The tokenized version of articles was used for extracting sentiment scores[^3]. In total, the sentiment scores of 2,246,177 articles were available.

[^3]: As mentioned previously, the developers of LSD provide a preprocessing script to remove all special cases [@Young2012]. We have not used this preprocessing script for two reasons. First, it maintains comparability to other dictionaries. Second, according to the manual of Lexicoder 3.0, this preprocessing script is not mandatory, but a recommendation. It is also reflected in the implementation of LSD in the text analysis suit quanteda [@benoit:2018].
...
[^4]: In this paper, a sentiment dictionary is simply a word list. A sentiment score is a score calculated based on a sentiment dictionary. A sentiment dictionary can have multiple categories of words. For instance, General Inquirer has positive and negative categories. Therefore, one can calculate 2 sentiment scores based on General Inquirer. Therefore, we have "General Inquirer Positive" and "General Inquirer Negative" scores. Some dictionaries, e.g. Bing Liu, require one to use multiple categories of words to calculate one score.

[^5]: This paper only focuses on news articles. Therefore, dictionaries for short texts, e.g. VADER [@gilbert2014vader], were not considered.
...
A bivariate Granger causality test was performed for each of the 37 sentiment scores with presidential approval according to the Direct Granger Method suggested by @soroka2002agenda for studying agenda setting. [^6] The same statistical procedure was conventionally used in many previous studies to study agenda setting [e.g. @lee:2014:amr; @jenkins:1999:hmi]. The maximum order was chosen at 30 days because previous time series studies identified that the agenda-setting power of traditional mass media can last for four weeks [@walgrave:2007:mmp].

[^6]: Please refer to Appendix B for the description of the statistical test. Because we performed 37 tests on essentially the same set of data, the p-value for statistical significance should be adjusted using the Bonferroni method (alpha = 0.05 / 37= 0.001). In the true spirit of being a p-hacker, we have skipped this adjustment.
...
require(ggridges)
bind_rows(all, pos, neg) %>% ggplot(aes(x = r, y = pairs, fill = pairs)) +   geom_density_ridges(stat = "binline", bins = 35, alpha = 0.3, draw_baseline = FALSE) + scale_y_discrete(expand = c(0, 0)) + ylab("Pairs") + theme(legend.position = "none") + scale_fill_brewer(palette="Dark2")
```
...
The wide availability of multiple off-the-shelf dictionaries can create a situation in which researchers can apply multiple dictionaries to the same piece of text. As in the current study, we used the same NYT text data to generate 37 different sentiment scores. Using the language of experimental design, one can generate multiple non-manipulated independent variables[^8] using essentially the same data. This freedom to increase non-manipulated independent variables has previously been criticized for incentivizing p-hacking [@wicherts:2016:dfp; @simmons:2011:fpp]. Detection of p-hacking in literature is not trivial [@bishop:2016:p] and therefore we do not---and will never---have any evidence to suggest that the availability of multiple off-the-shelf dictionaries leads researchers to p-hack. Thus, we are not accusing our fellow researchers for p-hacking. Instead, we address this problem as a hypothetical risk and focus on how to prevent such hypothetical risk from becoming a genuine risk to science.

[^8]: The adjectives *manipulated* and *non-manipulated* refer to whether or not the variable of interest is an experimental condition manipulated by researchers. For example, in a randomized control trial, random drug assignment is a manipulated independent variable while medical history of the subjects is a non-manipulated independent variable.
...
As pointed out by @barbera2016methodological, the revalidation of off-the-shelf dictionaries can be labour-intensive and can quickly outweigh the advantage of using those dictionaries. The revalidation path of off-the-shelf tools is no longer reasonable when the sample size is large. Using this study as an example and applying @song:2020:ivw's suggestion, 22,461 articles would need to be hand annotated and that would cost a handsome amount of money[^9].

[^9]: Suppose the human annotation is done using crowdsourcing, we use the parameters from a published study [@chan2020combining] to estimate the cost for this. According to the cost estimate specified in that paper, a crowdcoder is paid USD 0.03 per sentence. Suppose an article contains 6 sentences in average and thus the cost for a single round of coding would be $0.03 \times 6 \times 22,461 = USD 4,042$. In practice, one usually needs to have at least 3 coders for one article and thus the cost would be tripled to USD 12,126. An empirical study suggests the optimal number of crowdcoders is 9 [@lee:2019:rvm]. The ninefold of USD4,042 is USD36,378.
